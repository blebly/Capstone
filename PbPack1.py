...
class MultipleChoiceWindow(Screen):
    #code: 문제코드 받기
    #answer: 사용자입력 답안
    code = CF1pb[0]['code']
    ...
    def pb(self,cpt):
        print("cpt: ",cpt)
        cptlist = db.random(cpt)
        # algorithm
        #(0: default pb)/ 123: provide pb - 101
        print('cptlist ===========', cptlist)
        for i in range(len(cptlist)):
            if self.num == i:
                self.code = CF1pb[cptlist[i]]['code']
                self.ids.problem.source = CF1pb[cptlist[i]]['img']
                # self.tagLabel.text = CF1pb[cptlist[i]]['taglabel']
                i = i + 1
                sm.current = "multiplechoice"
            elif self.num == len(cptlist):
                print("========= DONE =========")
                i = 0       # when pass cpt
                break
...
screens = [MultipleChoiceWindow(name="multiplechoice"), ShortAnswerWindow(name="shortanswer"),ResultPassWindow(name="rpass"),ResultAgainWindow(name="ragain")]
